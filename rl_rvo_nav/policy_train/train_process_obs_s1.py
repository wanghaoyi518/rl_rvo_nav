...
world_abs_path = str(cur_path/'mode7_stage1_simple.yaml')

# 默认参数
counter = 0

parser = argparse.ArgumentParser(description='Mode 7 Stage 1 Curriculum Learning - Simple Random Polygons')

# 环境参数组
par_env = parser.add_argument_group('par env', 'environment parameters') 
par_env.add_argument('--env_name', default='mrnav-v1')
par_env.add_argument('--world_path', default='mode7_stage1_simple.yaml')  # Stage 1配置
...
par_train.add_argument('--train_epoch', type=int, default=300)  # Stage 1: 较短训练周期
...
par_train.add_argument('--save_name', default='r4_mode7_stage1_')  # Stage 1模型命名
par_train.add_argument('--load_path', default=str(cur_path / 'model_save')+ '/')
par_train.add_argument('--load_name', default='r4_1/r4_1_check_point_150.pt')  # 从base model开始
...
par_curriculum.add_argument('--curriculum_stage', default=1, type=int, help='Current curriculum stage (1-3)')
...
print("🎯 Mode 7 Stage 1 课程学习训练")
print("=" * 60)
print(f"📁 加载基础模型: {load_fname}")
